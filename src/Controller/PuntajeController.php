<?php
declare(strict_types=1);

namespace App\Controller;

/**
 * Puntaje Controller
 *
 * @property \App\Model\Table\PuntajeTable $Puntaje
 * @method \App\Model\Entity\Puntaje[]|\Cake\Datasource\ResultSetInterface paginate($object = null, array $settings = [])
 */
class PuntajeController extends AppController
{
    /**
     * Index method
     *
     * @return \Cake\Http\Response|null|void Renders view
     */
    public function index()
    {
        $puntaje = $this->paginate($this->Puntaje);

        $this->set(compact('puntaje'));
    }
    public function inicio()
    {
        $puntaje = $this->paginate($this->Puntaje);

        $this->set(compact('puntaje'));
    }

    /**
     * View method
     *
     * @param string|null $id Puntaje id.
     * @return \Cake\Http\Response|null|void Renders view
     * @throws \Cake\Datasource\Exception\RecordNotFoundException When record not found.
     */
    public function view($id = null)
    {
        $puntaje = $this->Puntaje->get($id, [
            'contain' => [],
        ]);

        $this->set(compact('puntaje'));
    }

    /**
     * Add method
     *
     * @return \Cake\Http\Response|null|void Redirects on successful add, renders view otherwise.
     */
    public function add()
    {
        $puntaje = $this->Puntaje->newEmptyEntity();
        if ($this->request->is('post')) {
            $puntaje = $this->Puntaje->patchEntity($puntaje, $this->request->getData());
            if ($this->Puntaje->save($puntaje)) {
                $this->Flash->success(__('The puntaje has been saved.'));

                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error(__('The puntaje could not be saved. Please, try again.'));
        }
        $this->set(compact('puntaje'));
    }

    /**
     * Edit method
     *
     * @param string|null $id Puntaje id.
     * @return \Cake\Http\Response|null|void Redirects on successful edit, renders view otherwise.
     * @throws \Cake\Datasource\Exception\RecordNotFoundException When record not found.
     */
    public function edit($id = null)
    {
        $puntaje = $this->Puntaje->get($id, [
            'contain' => [],
        ]);
        if ($this->request->is(['patch', 'post', 'put'])) {
            $puntaje = $this->Puntaje->patchEntity($puntaje, $this->request->getData());
            if ($this->Puntaje->save($puntaje)) {
                $this->Flash->success(__('The puntaje has been saved.'));

                return $this->redirect(['action' => 'index']);
            }
            $this->Flash->error(__('The puntaje could not be saved. Please, try again.'));
        }
        $this->set(compact('puntaje'));
    }

    /**
     * Delete method
     *
     * @param string|null $id Puntaje id.
     * @return \Cake\Http\Response|null|void Redirects to index.
     * @throws \Cake\Datasource\Exception\RecordNotFoundException When record not found.
     */
    public function delete($id = null)
    {
        $this->request->allowMethod(['post', 'delete']);
        $puntaje = $this->Puntaje->get($id);
        if ($this->Puntaje->delete($puntaje)) {
            $this->Flash->success(__('The puntaje has been deleted.'));
        } else {
            $this->Flash->error(__('The puntaje could not be deleted. Please, try again.'));
        }

        return $this->redirect(['action' => 'index']);
    }

}
